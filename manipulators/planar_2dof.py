...
class PlanarManipulator2DOF:
    ...
    def M(self, x):
        ...
        alpha = self.I_1 + self.I_2 + self.I_3 + self.m1 * pow(self.d1, 2) + self.m2 * (pow(self.l1, 2) + pow(self.d2, 2)) + 2 * self.m2 * self.l1 * self.d2 * mh.cos(q2) + self.m3 * (pow(self.l1, 2) + pow(self.l2, 2)) + 2 * self.m3 * self.l1 * self.l2 * mh.cos(q2)
        gamma = self.m2 * pow(self.d2, 2) + self.m3 * (pow(self.l2, 2)) + self.I_2 + self.I_3
        beta = self.m2*self.l1*self.d2 + self.m3*self.l1*self.l2

        Mass = np.array([alpha+2*beta*mh.cos(q2), gamma+(beta*mh.cos(q2))], [gamma+(beta*mh.cos(q2)), gamma])
    ...
    def C(self, x):
        ...
        Coriolis = np.array([-1 * beta * mh.sin(q2) * q2_dot, -1 * beta * mh.sin(q2) * (q1_dot + q2_dot)],[beta * mh.sin(q2) * q1_dot, 0])
